...
public class FrinikaSequencerPlayer implements Runnable {
    ...
    void timerEvent() {
        ...
        if (sequence.getDivisionType() == FrinikaSequence.PPQ) {
            /**
             * CurrentTick is the time in ticks - regardless of looping - since
             * the playback was started WARNING (PJS): This is not the same as
             * the sequencer tick position
             */
            long currentTick;
            ...
            for (long playTick = lastTickPosition; playTick <= currentTick; playTick++) {
                ...
                for (FrinikaTrackWrapper trackWrapper : ftws) {
                    ...
                    if (first) { // first track is tempo track
                        List<MidiEvent> events = trackWrapper
                    ...
                    if (trackWrapper.getMidiDevice() != null && !opt.muted) {
                        ...
                        try {
                            ...
                            if (opt.quantizationActive) {
                                ...
                                List<MidiEvent> eventsToBeQuantized = trackWrapper
    ...
    private void handleTempoEvents(List<MidiEvent> events) {
        for (MidiEvent evt : events) {
            // Handle tempomessages seperately
            byte[] msgBytes = evt.getMessage().getMessage();
            if (msgBytes[0] == -1 && msgBytes[1] == 0x51 && msgBytes[2] == 3) {
                ...
                if (realtime) {
                    /**
                     * (PJS) Since the sequencer player calculates the time in
                     * ticks based on the startTickPosition and the tempo, the
                     * startTickPosition has to be altered to be the same as the
                     * tick position when the tempo change occured.
                     */
    ...
    private static void quantizeOnTheFly(List<MidiEvent> eventsToBeQuantized,
        ...
        for (MidiEvent event : eventsToBeQuantized) {
            ...
            List<MidiEvent> v = quantizeBuffer.data[(int) d];
            if (v == null) {
                v = new ArrayList<>();
    ...
    private static void rebufferQuantization(FrinikaTrackWrapper trackWrapper,
        ...
        for (long tick = from; tick < to; tick++) {
            List<MidiEvent> events = trackWrapper.getEventsForTick(tick);
            if (events != null) {
...
//						+ tempoList);
    ...
    private long getLoopedTick(long tick, FrinikaTrackWrapper trackWrapper,
            MidiPlayOptions opt) {
        int s = trackWrapper.getTrack().size();
        if (s <= 1) {
            return tick;
        }
    ...
    /**
     * The delay due to audio latency which must be subtracted when recording
     * midi events
     *
