...
/**
 * The EditHistoryContainer monitors all edits on a EditHistoryRecorders. By
 * setting a marking pont using the mark method - it will be possible to roll
 * back to the state at the marking point at a later stage. It's also possible
 * to roll back and roll forward again. This gives undo/redo features to a
 * EditHistoryRecorder.
 *
...
public class EditHistoryContainer {
    ...
    private final List<EditHistoryAction> editHistory = new LinkedList<>();

    private final List<EditHistoryMark> editHistoryMarks = new LinkedList<>();
    ...
    private final List<EditHistoryListener> editHistoryListeners = new LinkedList<>();

    /**
     * Set to false during undo or redo operations so that these operations are
    ...
    public void notifyEditHistoryListeners() {
        List<EditHistoryAction> editHistoryActions = new LinkedList<>();
        ...
        /**
         * TODO: PJS: What is this? Yes it can be zero, since you may undo
         * everything... Why shouldn't it?
         *
         * if size is zero then nothing has happened ?
         *
         * PJL: OK if everything can cope with it being zero I commented it out
         * because some stuff was falling over. array out of bounds if I recall
         * correctly. Maybe fixed now ? if (editHistoryActions.size() == 0 ) {
         * try { throw new Exception(" Should the editHistory really be zero ");
    ...
    /**
     * Tells if there has been any edits on this history container.
     *
     * @return
     */
    public boolean hasChanges() {
        return redoMarkIndex != savedPosition;
    }
    ...
    /**
     * Use during undo/redo operations so that these are not recorded in the
     * EditHistory
     */
    ...
    /**
     * Called by the ProjectContainer when saving the project so that
     * hasChanges() will return false as long as there are no changes after the
     * save
     *
     */
