...
public class FrinikaJackAudioServer implements AudioServer {

    private final static Logger LOG = Logger.getLogger(FrinikaJackAudioServer.class.getName());
    ...
    private FrinikaJackAudioServer(String id, AudioConfiguration ctxt,
    ...
    private void connectInputs() {
        try {
            ...
            Logger.getLogger(FrinikaJackAudioServer.class.getName()).log(Level.SEVERE, null, ex);
        }
     ...
     * @param ctxt Requested audio configuration. The samplerate and buffer
     * settings of the context will be ignored. These settings are taken from
     * Jack itself.
     * @param autoconnect Whether to connect inputs and outputs to first found
     * physical ports
     * @param client Audio client to process every Jack callback.
     * @return server
     */
    @Deprecated
    public static FrinikaJackAudioServer create(String id, AudioConfiguration ctxt,
            boolean autoconnect, AudioClient client) {
        if (id == null || ctxt == null || client == null) {
            throw new NullPointerException();
        }
        return new FrinikaJackAudioServer(id, ctxt, autoconnect, client);
    }
}
